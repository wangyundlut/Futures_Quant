...
class backtest_ma:
    ...
    def spe_time_list(self):
        ...
        time_list = [
                     ...
                     ['2014-12-12 09:00:00', '2018-08-21 15:30:00'],
                     ]
    ...
    def period_strategy_list(self):
        ...
        strategy_list = [[20, 25, 30, 35, 40, 45, 50, 55, 60],[6, 8, 10, 12, 14, 16]]
    ...
    def min_unit(self, backtest_data, strategy_dict, backtest_dict):
        ...
        # 策略类
        class_strategy = strategy_ema(fast, slow)
        # 回测类
        ...
        for i in range(0, len(backtest_data)):
            ...
            if datalen >= class_strategy.slow_index + 10:
                # 计算仓位：以固定1手交易
                open_num = 1
                # 首次处理
                if datalen == class_strategy.slow_index + 10:
                    if class_strategy.slow_ema[-1] >= class_strategy.fast_ema[-1]:
    ...
    def main(self):
        ...
        for i in range(0, len(spe_list)):
            ...
            for slow in strategy_list[0]:
                ...
                strategy_name = 'ema_' + spe + '_' + str(slow) + '_' + str(strategy_list[1][0])
                # 分析类
                class_analysis = BackTestAnalysis(strategy_name)
                # 查看本地是否有文件，如果没有文件，继续
                ...
                # 回测参数
                strategy_name1 = 'ema_' + spe + '_' + str(slow) + '_' + str(strategy_dict1['fast_index'])
                strategy_name2 = 'ema_' + spe + '_' + str(slow) + '_' + str(strategy_dict2['fast_index'])
                strategy_name3 = 'ema_' + spe + '_' + str(slow) + '_' + str(strategy_dict3['fast_index'])
                strategy_name4 = 'ema_' + spe + '_' + str(slow) + '_' + str(strategy_dict4['fast_index'])
                strategy_name5 = 'ema_' + spe + '_' + str(slow) + '_' + str(strategy_dict5['fast_index'])
                strategy_name6 = 'ema_' + spe + '_' + str(slow) + '_' + str(strategy_dict6['fast_index'])

                backtest_dict1 = {}
                backtest_dict1['strategy_name'] = strategy_name1
                backtest_dict1['initial_money'] = initialmoney
                backtest_dict1['slip'] = slip
                backtest_dict2 = copy.deepcopy(backtest_dict1)
                backtest_dict2['strategy_name'] = strategy_name2
                backtest_dict3 = copy.deepcopy(backtest_dict1)
                backtest_dict3['strategy_name'] = strategy_name3
                backtest_dict4 = copy.deepcopy(backtest_dict1)
                backtest_dict4['strategy_name'] = strategy_name4
                backtest_dict5 = copy.deepcopy(backtest_dict1)
                backtest_dict5['strategy_name'] = strategy_name5
                backtest_dict6 = copy.deepcopy(backtest_dict1)
                backtest_dict6['strategy_name'] = strategy_name6
                ...
                # self.min_unit(backtest_data, strategy_dict1, backtest_dict1)
