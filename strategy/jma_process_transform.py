...
class jmavalue:
    def __init__(self, data, Length, phase):
        self.initFlag = True
        self.data = data
        self.Length = Length
        self.phase = phase
        nr = self.data.shape[0]
        self.JMAValueBuffer = pd.DataFrame(np.zeros([nr, 1]))


    def jma_calculate(self):
        print(str(datetime.datetime.now()) +' 正在计算JMAValue，请稍等......')
        ...
        for shift in range(0, nr):
            # 实盘部署中，此处应加入线程的停顿，每隔一段时间，给self.data重置一个数据，计算JMA的当时的两个值，然后就可以比较了
            series = self.data.iloc[shift, 0]
            if loopParam < 61:
                loopParam += 1
            ...
            if loopParam > 30:
                ...
                if loopCriteria > 30:
                    JMATempValue = self.s19(fC0Buffer,fA8Buffer,fC8Buffer,self.JMAValueBuffer,shift,lengthDivider, dValue, series,phaseParam)
            self.JMAValueBuffer.iloc[shift, 0] = JMATempValue

            if nr % 1000 == 0:
                print(str(nr/1000) + ' in ', str(nr))
        x = self.JMAValueBuffer
        print(str(datetime.datetime.now()) + ' JMAValue计算完毕！！！')
        return x
...
if __name__=='__main__':
    ...
    self = jmavalue(data,Length=21 ,phase=101)
